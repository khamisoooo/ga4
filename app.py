...
# ── WINDSOR SETTINGS ─────────────────────────────────────────
# 🔧 ضع الـ Account IDs هنا بعد ما تجيبهم من Windsor
ACCOUNTS = {
    "web": {
        "label": "🌐 Website",
        "account_id": "292247905",
    },
    "app": {
        "label": "📱 Mobile App",
        "account_id": "248305344",
    },
}
...
def fetch(account_id, fields, date_from, date_to, timeout=30):
    """Fetch data from Windsor for a specific account."""
    if not WINDSOR_KEY:
        return pd.DataFrame()
    params = {
        "api_key":    WINDSOR_KEY,
    ...
    try:
        ...
        st.error(f"Windsor Error: {e}")
        return pd.DataFrame()

def merge_sources(df_web, df_app, group_cols):
    """Merge web + app DataFrames, tag source, combine."""
    frames = []
    if not df_web.empty:
        df_web = df_web.copy(); df_web["source"] = "web"
        frames.append(df_web)
    if not df_app.empty:
        df_app = df_app.copy(); df_app["source"] = "app"
        frames.append(df_app)
    if not frames:
        return pd.DataFrame()
    combined = pd.concat(frames, ignore_index=True)
    num_cols = [c for c in combined.columns if c not in group_cols + ["source"]]
    for c in num_cols:
...
def aggregate(df, group_cols):
    ...
    if df.empty:
        return df
    num_cols = [c for c in df.columns if c not in group_cols + ["source"]]
...
def export_btn(df, filename):
    ...
    st.download_button("📥 Export CSV", buf.getvalue(), filename, "text/csv")
...
.src-all{background:#F0EEF8;color:#534AB7;padding:2px 7px;border-radius:6px;font-size:10px;font-weight:600}
...
with st.sidebar:
    ...
    <div style="font-size:17px;font-weight:700;color:#1A1A2E">
      <span style="color:#3266AD">●</span> Analytics
    </div>
    ...
    # ── DATE RANGE ───────────────────────────────────────────
    st.markdown('<div style="font-size:11px;font-weight:600;color:#73726C;text-transform:uppercase;letter-spacing:.05em;margin-bottom:6px">Date Range</div>', unsafe_allow_html=True)
    ...
    # ── NAV ──────────────────────────────────────────────────
    tab = st.radio("", [
        "📊 Overview", "🔽 Funnel", "📡 Traffic",
        "📱 Devices", "🛒 E-Commerce", "🎯 Campaigns",
        "👥 Users", "💡 Insights"
    ], label_visibility="collapsed")
    ...
    st.markdown(f'<div style="font-size:10px;color:#9A9A8E;line-height:1.7">'
                ...
                f'{d_from} → {d_to}<br>'
                f'<span style="color:#1D9E75">● Live</span> · Windsor GA4</div>', unsafe_allow_html=True)
...
if not WINDSOR_KEY:
    st.markdown("""<div style="text-align:center;padding:60px 20px">
      ...
      <div style="font-size:18px;font-weight:600;color:#1A1A2E;margin-bottom:8px">Add Windsor API Key</div>
      <div style="font-size:13px;color:#9A9A8E">
        Streamlit → Settings → Secrets<br>
        <code>WINDSOR_API_KEY = "your_key_here"</code>
      </div>
    </div>""", unsafe_allow_html=True)
    st.stop()
...
def load(account_id, fields, d_from, d_to, timeout=30):
    return fetch(account_id, fields, d_from, d_to, timeout)

def load_combined(fields, d_from, d_to, group_cols=None, timeout=30):
    """Load from web + app based on filter, return combined df."""
    if source_filter == "web":
        df = load(ACCOUNTS["web"]["account_id"], fields, d_from, d_to, timeout)
        if not df.empty: df["source"] = "web"
        return df
    elif source_filter == "app":
        df = load(ACCOUNTS["app"]["account_id"], fields, d_from, d_to, timeout)
        if not df.empty: df["source"] = "app"
        return df
    else:  # all
        df_w = load(ACCOUNTS["web"]["account_id"], fields, d_from, d_to, timeout)
        df_a = load(ACCOUNTS["app"]["account_id"], fields, d_from, d_to, timeout)
        return merge_sources(df_w, df_a, group_cols or [])
...
def sh(t, s="", c="#3266AD"):
    ...
    <div class="st_">{t}</div>
    {'<div class="ss_">'+s+'</div>' if s else ''}</div>"""
...
def src_badge(source):
    if source == "web": return '<span class="src-web">🌐 Web</span>'
    if source == "app": return '<span class="src-app">📱 App</span>'
    return '<span class="src-all">🌐📱 All</span>'
...
with st.spinner("⏳ Loading data..."):
    df_ov1 = load_combined(["date","sessions","active_users","bounce_rate","average_session_duration"], d_from, d_to, ["date"])
    df_ov2 = load_combined(["date","purchase_revenue","transactions","add_to_carts","checkouts"], d_from, d_to, ["date"])

# Merge overview
if not df_ov1.empty and not df_ov2.empty:
    try:
        merge_keys = ["date","source"] if "source" in df_ov1.columns and "source" in df_ov2.columns else ["date"]
...
if df_ov.empty:
    st.error("❌ No data — check Account IDs in app.py and API Key in Secrets.")
    st.stop()
...
if tab == "📊 Overview":
    ...
    # Source comparison if "all"
    if source_filter == "all" and "source" in df_ov.columns:
        df_w = df_ov[df_ov["source"]=="web"]; df_a = df_ov[df_ov["source"]=="app"]
        ...
        with sc1:
            st.markdown(f"""<div style="background:#E6F1FB;border-radius:10px;padding:12px 16px;border-left:4px solid #3266AD">
            <div style="font-size:11px;color:#185FA5;font-weight:600;margin-bottom:6px">🌐 Website</div>
            <div style="display:flex;gap:20px">
              <div><div style="font-size:10px;color:#73726C">Sessions</div><div style="font-size:18px;font-weight:600;color:#1A1A2E">{fmt_n(w_ses)}</div></div>
              <div><div style="font-size:10px;color:#73726C">Revenue</div><div style="font-size:18px;font-weight:600;color:#1D9E75">{fmt_c(w_rev)}</div></div>
            </div></div>""", unsafe_allow_html=True)
        with sc2:
            st.markdown(f"""<div style="background:#EAF7F2;border-radius:10px;padding:12px 16px;border-left:4px solid #1D9E75">
            <div style="font-size:11px;color:#0D6B4F;font-weight:600;margin-bottom:6px">📱 Mobile App</div>
            <div style="display:flex;gap:20px">
              <div><div style="font-size:10px;color:#73726C">Sessions</div><div style="font-size:18px;font-weight:600;color:#1A1A2E">{fmt_n(a_ses)}</div></div>
              <div><div style="font-size:10px;color:#73726C">Revenue</div><div style="font-size:18px;font-weight:600;color:#1D9E75">{fmt_c(a_rev)}</div></div>
            </div></div>""", unsafe_allow_html=True)
    ...
    if "date" in df_ov.columns:
        ...
        # Aggregate by date (combine sources if needed)
    ...
    # Web vs App comparison chart
    if source_filter == "all" and "source" in df_ov.columns and "date" in df_ov.columns:
        st.markdown("<div style='height:8px'></div>", unsafe_allow_html=True)
        st.markdown(sh("Web vs App — Daily Revenue","","#7F77DD"), unsafe_allow_html=True)
        df_ts2 = df_ov.copy()
        df_ts2["date"] = pd.to_datetime(df_ts2["date"], errors="coerce")
        df_ts2 = df_ts2.dropna(subset=["date"]).sort_values("date")
        df_web_ts = df_ts2[df_ts2["source"]=="web"].groupby("date")["purchase_revenue"].sum().reset_index()
        df_app_ts = df_ts2[df_ts2["source"]=="app"].groupby("date")["purchase_revenue"].sum().reset_index()
        fig3 = go.Figure()
        if not df_web_ts.empty:
            fig3.add_trace(go.Scatter(x=df_web_ts["date"],y=df_web_ts["purchase_revenue"]/1000,name="🌐 Web",line=dict(color="#3266AD",width=2),fill="tozeroy",fillcolor="rgba(50,102,173,0.1)"))
        if not df_app_ts.empty:
            fig3.add_trace(go.Scatter(x=df_app_ts["date"],y=df_app_ts["purchase_revenue"]/1000,name="📱 App",line=dict(color="#1D9E75",width=2),fill="tozeroy",fillcolor="rgba(29,158,117,0.1)"))
        fig3.update_layout(**PLOT_LAYOUT,height=220,legend=dict(orientation="h",y=1.02,x=1,xanchor="right"))
...
# ══════════════════════════════════════════════════════════════
    ...
    if source_filter == "all" and "source" in df_ov.columns:
        st.markdown("<div style='height:12px'></div>", unsafe_allow_html=True)
        st.markdown(sh("Web vs App Funnel","مقارنة","#7F77DD"), unsafe_allow_html=True)
        funnel_rows = []
        for src, label, badge in [("web","Website","src-web"),("app","Mobile App","src-app")]:
            d = df_ov[df_ov["source"]==src]
            if d.empty: continue
            s=d["sessions"].sum(); r=d["purchase_revenue"].sum()
            c=d["add_to_carts"].sum(); ck=d["checkouts"].sum(); o=d["transactions"].sum()
            _cvr=o/s*100 if s>0 else 0; _aov=r/o if o>0 else 0
            funnel_rows.append(f"""<tr>
              <td><span class="{badge}">{label}</span></td>
              <td>{fmt_n(s)}</td><td>{fmt_n(c)}</td><td>{fmt_n(ck)}</td>
        ...
        if funnel_rows:
            st.markdown(f"""<table><thead><tr>
              <th>Source</th><th>Sessions</th><th>Add to Cart</th><th>Checkout</th>
              <th>Orders</th><th>CVR</th><th>AOV</th>
            </tr></thead><tbody>{''.join(funnel_rows)}</tbody></table>""", unsafe_allow_html=True)
            export_btn(df_ov.groupby("source")[["sessions","purchase_revenue","transactions","add_to_carts","checkouts"]].sum().reset_index(), "funnel_comparison.csv")
...
# ══════════════════════════════════════════════════════════════
    ...
    with st.spinner(""):
        df_ch_raw = load_combined(
            ...
            d_from, d_to, ["session_default_channel_group"]
        )
    ...
    if not df_ch_raw.empty and "session_default_channel_group" in df_ch_raw.columns:
        ...
        # Show per-source breakdown if "all"
        if source_filter == "all" and "source" in df_ch.columns:
            st.markdown('<div style="margin-bottom:8px">', unsafe_allow_html=True)
            src_tabs = st.tabs(["🌐📱 Combined", "🌐 Web", "📱 App"])
            for idx, (src_filter_val, src_df) in enumerate([
                ("all",  df_ch),
                ("web",  df_ch[df_ch["source"]=="web"] if "source" in df_ch.columns else df_ch),
                ("app",  df_ch[df_ch["source"]=="app"] if "source" in df_ch.columns else pd.DataFrame()),
            ]):
                with src_tabs[idx]:
                    if src_df.empty:
                    ...
                    export_btn(dg[["session_default_channel_group","sessions","purchase_revenue","transactions"]],f"channels_{src_filter_val}.csv")
        else:
            dg=df_ch.groupby("session_default_channel_group").sum(numeric_only=True).reset_index()
            dg=dg[dg["sessions"]>5].sort_values("purchase_revenue",ascending=False)
            cl2,cr2=st.columns(2)
            with cl2:
            ...
            with cr2:
                ...
                for i,(_,r) in enumerate(dg.sort_values("purchase_revenue",ascending=False).head(8).iterrows()):
                    st.markdown(bar(r["session_default_channel_group"],r["purchase_revenue"]/mx*100 if mx else 0,COLORS[i%len(COLORS)],fmt_c(r["purchase_revenue"])),unsafe_allow_html=True)
            export_btn(dg[["session_default_channel_group","sessions","purchase_revenue","transactions"]],"channels.csv")
...
# ══════════════════════════════════════════════════════════════
    ...
    if not df_dv_raw.empty and "devicecategory" in df_dv_raw.columns:
        ...
        if source_filter=="all" and "source" in df_dv_raw.columns:
            st.markdown(sh("Web vs App — Device Split","","#7F77DD"), unsafe_allow_html=True)
            for src,lbl in [("web","🌐 Website"),("app","📱 App")]:
                d=df_dv_raw[df_dv_raw["source"]==src]
                if d.empty: continue
                d=d[d["devicecategory"].isin(["mobile","desktop","tablet"])]
                st.markdown(f"**{lbl}**")
                cols=st.columns(len(d)) if len(d)>0 else st.columns(1)
                for i,(_,r) in enumerate(d.iterrows()):
                    dev=r["devicecategory"]; clr=DC.get(dev,"#888780")
                    br=r["bounce_rate"]*100; sp=r["sessions"]/d["sessions"].sum()*100 if d["sessions"].sum()>0 else 0
                    with cols[i]: st.markdown(kpi(dev.title(),f"{sp:.1f}%",f"Bounce: {br:.1f}%","dn" if br>55 else "wn" if br>45 else "up",f"Rev: {fmt_c(r['purchase_revenue'])}",c=clr),unsafe_allow_html=True)
                st.markdown("<div style='height:8px'></div>",unsafe_allow_html=True)
        else:
            ...
            for i,(_,r) in enumerate(md.iterrows()):
                dev=r["devicecategory"]; clr=DC.get(dev,"#888780")
                br=r["bounce_rate"]*100; sp=r["sessions"]/md["sessions"].sum()*100 if md["sessions"].sum()>0 else 0
                with cols[i]: st.markdown(kpi(dev.title(),f"{sp:.1f}%",f"Bounce: {br:.1f}%","dn" if br>55 else "wn" if br>45 else "up",f"Rev: {fmt_c(r['purchase_revenue'])}",c=clr),unsafe_allow_html=True)
...
# ══════════════════════════════════════════════════════════════
    ...
    with st.spinner(""):
        df_gads = load_combined(
            ...
            d_from, d_to, ["session_google_ads_campaign_name"]
        )
        df_meta = load_combined(
            ["session_manual_campaign_name","sessions","purchase_revenue","transactions","add_to_carts"],
            d_from, d_to, ["session_manual_campaign_name"]
        )

    camp_type_tabs = st.tabs(["🟦 Google Ads", "🟦 Meta Campaigns"])

    # Google Ads
    with camp_type_tabs[0]:
...
# ══════════════════════════════════════════════════════════════
    ...
    with st.spinner("Loading cohort (90d)..."):
        ...
        df_coh = load_combined(["date","new_vs_returning","sessions","purchase_revenue","transactions"], c_from, c_to, ["date","new_vs_returning"])
...
# ══════════════════════════════════════════════════════════════
    ...
    if source_filter == "all":
        st.markdown(ins("⚠","مقارنة Web vs App","شوف الـ Funnel tab للمقارنة التفصيلية بين الويب والتطبيق واتخذ قرارات الـ budget بناءً عليها.","ib"),unsafe_allow_html=True)
